...
class PropertywalasalehyderabadSpider(scrapy.Spider):
    name = "propertywalaSaleHyderabad"
    allowed_domains = ["propertywala.com"]
    start_urls = (
        'https://www.propertywala.com/properties/type-residential/for-sale/location-hyderabad_andhra_pradesh?page=%s' % page for page in range(1, 41)
    )
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 1}
    ...
    def parse(self, response):
        ...
        for i in data:
            ids = i.xpath('text()').extract_first().strip().encode('ascii', 'ignore').decode('ascii').replace('ID: ','').replace(
                '  Posted:', '')
    ...
    def parse1(self, response):
        ...
        if 'plot' in checkprop.lower() or 'land' in checkprop.lower():
            ...
            try:
                ...
                if 'lakh' in price_square:
                    price_square = price_square.split(' la')[0]
                    self.item['price_per_sqft'] = str(float(price_square.strip()) * 100000)
                elif 'crore' in price_square.lower():
                    price_square = price_square.split(' crore')[0]
                    self.item['price_per_sqft'] = str(float(price_square.strip()) * 10000000)
                else:
