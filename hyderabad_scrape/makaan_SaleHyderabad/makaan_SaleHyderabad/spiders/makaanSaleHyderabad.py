...
class MakaansalehyderabadSpider(scrapy.Spider):
    name = "makaanSaleHyderabad"
    allowed_domains = ["makaan.com"]
    start_urls = (
        'https://www.makaan.com/listings?listingType=buy&pageType=LISTINGS_PROPERTY_URLS&cityName=Hyderabad&cityId=12&templateId=MAKAAN_CITY_LISTING_BUY/',
    )
    custom_settings = {
        'DEPTH_LIMIT': 5000,
        'DOWNLOAD_DELAY': 3,
    ...
    def parse(self, response):
        ...
        for i in a:
            ...
            if item['Building_name'] == '' or item['Building_name'] == '-':
                item['Building_name'] = 'None'
            ...
            if 'ale' in item['txn_type']:
                prc_pr_sf = i.xpath(
                    'div[@class="cardWrapper"]/div[@class="cardLayout clearfix"]/div[@class="infoWrap"]/div[@class="headInfo"]/div[@class="priceWrap"]/div[@class="price-rate-col"]/div[@class="rate"]/span[@class="val"]/text()').extract_first()
