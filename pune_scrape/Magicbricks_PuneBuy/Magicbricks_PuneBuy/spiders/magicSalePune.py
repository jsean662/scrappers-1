...
=======
            ...
            if 'resultBlockWrapper' in str(response.body):
                pageNo = int(response.url.split('Page-')[1])
                url = 'http://www.magicbricks.com/property-for-sale/residential-real-estate?proptype=Multistorey-Apartment,Builder-Floor-Apartment,Penthouse,Studio-Apartment,Residential-House,Villa&cityName=Pune&sortBy=mostRecent/Page-' + str(pageNo + 1)
                # print(url)
                yield Request(url, callback=self.parse, dont_filter=True)
