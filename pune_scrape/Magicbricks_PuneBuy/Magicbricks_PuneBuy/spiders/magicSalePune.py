...
class MagicsalepuneSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://www.magicbricks.com/property-for-sale/residential-real-estate?proptype=Multistorey-Apartment,Builder-Floor-Apartment,Penthouse,Studio-Apartment,Residential-House,Villa&cityName=Pune/Page-10',
    )
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 5}
    ...
    def parse(self, response):
        ...
        for i in data:
            ...
            ids = re.findall('[0-9]+', ids)[0]
            item['data_id'] = ids
            try:
                projname = 'projectName'+ str(ids)

                item['Building_name'] = i.xpath('//*[@id="' + projname + '"]/@value').extract_first()
            ...
            try:
                item['lat'] = i.xpath('.//div/div[@class="srpColm2"]/div[@class="proColmleft"]/div[@class="proNameWrap proNameWrapBuy"]/div[@class="proNameColm1"]/span[@class="seeOnMapLink seeOnMapLinkBuy"]/span[@class="stopParentLink"]/@onclick').extract_first().split('&')[0].split('?')[-1].split("=")[-1]  # .xpath('.//input[@itemprop="latitude"]/@value').extract_first()
                if item['lat'] == '':
                    item['lat'] = '0'
            except:
                item['lat'] = '0'
            ...
            try:
                bedroomval = 'bedroomVal' + str(ids)
                print(bedroomval)
                item['config_type'] = i.xpath('//*[@id="' + bedroomval + '"]/@value').extract_first()# .replace('>', '')
            ...
            try:
                proparea = 'propertyArea' + str(ids)
                sqf = i.xpath('//*input[@id="propertyArea"]/@value').extract_first()
                if 'sqft' in sqf:
            ...
            if not price == None:
                ...
                if item['Selling_price'] == 'None' or item['Selling_price'] is None:
                    item['Selling_price'] = '0'
            else:
                item['Selling_price'] = '0'
            ...
            try:
                ...
                if (item['name_lister'] == '') or (item['name_lister'] == 'null') or item['name_lister'] is None:
                    item['name_lister'] = 'None'
            except:
                item['name_lister'] = 'None'

            transaction = 'transactionType' + str(ids)
            item['txn_type'] = i.xpath('//*[@id="' + transaction + '"]/@value').extract_first()

            createdate = 'createDate' + str(ids)
            day = i.xpath('//*[@id="' + createdate + '"]/@value').extract_first()
