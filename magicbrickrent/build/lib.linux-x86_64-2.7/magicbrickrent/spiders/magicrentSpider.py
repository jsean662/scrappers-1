...
class MagicrentSpider(scrapy.Spider):
	...
	def parse(self,response):
			...
			for i in data:
				...
				item['Building_name'] = i.xpath('div[@class="minHeightBlock"]/div[@class="proColmleft"]/div[@class="proNameWrap "]/div[@class="proNameColm1"]/div[@class="srpTopDetailWrapper"]/div[@class="srpAnchor"]/p[@class="proHeading"]/a/span[@class="maxProDesWrap showNonCurtailed"]/text()').extract_first()
				if (('for rent in' in item['Building_name']) and (',' in item['Building_name'])):
					item['Building_name'] = item['Building_name'].replace("for rent in","").replace(",","").strip()
				if ((item['Building_name'] == '') or (item['Building_name'] == None) or ('for rent in' in item['Building_name'])):
					item['Building_name'] = 'None'
				
				item['config_type'] = i.xpath('div[@class="minHeightBlock"]/div[@class="proColmleft"]/div[@class="proNameWrap "]/div[@class="proNameColm1"]/div[@class="srpTopDetailWrapper"]/div[@class="srpAnchor"]/p[@class="proHeading"]/a/input[contains(@id,"bedroomVal")]/@value').extract_first()+'BHK'
				...
				sq = i.xpath('div[@class="minHeightBlock"]/div[@class="proColmleft"]/div[@class="proNameWrap "]/div[@class="proNameColm1"]/div[@class="srpTopDetailWrapper"]/div[@class="srpAnchor"]/p[@class="proHeading"]/a/@href').extract_first()
				item['Bua_sqft'] = sq.split('/')[-1].split('-')[2]
				if 'Studio' in sq:
					item['Bua_sqft'] = sq.split('/')[-1].split('-')[0]
