# -*- coding: utf-8 -*-
import scrapy
from ..items import MakaanPunesaleItem
import json
import datetime
...
class MakaansalepuneSpider(scrapy.Spider):
    name = "makaanRentMumbai"
    allowed_domains = ["makaan.com"]
    start_urls = [
        'https://www.makaan.com/listings?postedBy=owner&sortBy=date-desc&listingType=rent&pageType=LISTINGS_PROPERTY_URLS&cityName=Mumbai&cityId=18&templateId=MAKAAN_CITY_LISTING_BUY&page=%s' % page for page in range(1, 19)
    ]
    custom_settings = {
        'DEPTH_LIMIT': 5000,
        'DOWNLOAD_DELAY': 3,
        ...
        for i in a:
            try:
                ...
                if 'onstruction' in item['Status']:
                    try:
                        date = i.xpath(
                            'div[@class="cardWrapper"]/div[@class="cardLayout clearfix"]/div[@class="infoWrap"]/div[@class="highlight-points"]/div[@class="dcol poss"]/div[1]/text()').extract_first()
                        item['Possession'] = dt.strftime(dt.strptime(date, '%b %Y'), '%m/%d/%Y %H:%M:%S')
                    except Exception as e:
                        print(e)
                elif 'esale' in item['txn_type']:
                    item['age'] = i.xpath(
                        'div[@class="cardWrapper"]/div[@class="cardLayout clearfix"]/div[@class="infoWrap"]/div[@class="highlight-points"]/div[@class="dcol poss"]/div[@class="val ''"]/text()').extract_first()
