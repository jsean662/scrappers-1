...
class KolkatapropertiesbuySpider(scrapy.Spider):
    name = "kolkatapropertiesBuy"
    allowed_domains = ["kolkataproperties.in"]
    start_urls = (
        'http://www.kolkataproperties.in/search_results.php?buy=yes&z_itemPage=80',
    )
    custom_settings = {
        'DEPTH_LIMIT': 5000,
        'DOWNLOAD_DELAY': 3,
    ...
    def parse(self, response):
        ...
        data = record.xpath('//div[@class="col-md-4 bottom_property"]')
        txn_type = build_name = ''

        for i in data:

            proptype = i.xpath('.//div/a[1]/h4/text()').extract_first()
            if 'land' in proptype.lower():
                continue
