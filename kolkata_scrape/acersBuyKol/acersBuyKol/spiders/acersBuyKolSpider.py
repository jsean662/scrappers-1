import scrapy
from acersBuyKol.items import AcersbuykolItem
...
class AcersbuykolspiderSpider(scrapy.Spider):
    name = "acersBuyKolkata"
    allowed_domains = ["http://www.99acres.com/"]
    start_urls = ['http://www.99acres.com/property-in-kolkata-ffid-page-%s?orig_property_type=1,4,2,90&search_type=QS&search_location=CP12&pageid=QS&keyword_orig=pune&property_type=1,4,2,90' % page for page in xrange(1,667)]
    custom_settings = {
            'DEPTH_LIMIT': 3000,
            'DOWNLOAD_DELAY': 15
        }
