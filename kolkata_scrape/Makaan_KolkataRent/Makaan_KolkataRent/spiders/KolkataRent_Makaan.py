...
class KolkatarentMakaanSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://www.makaan.com/listings?sortBy=date-desc&listingType=rent&pageType=LISTINGS_PROPERTY_URLS&cityName=Kolkata&cityId=16&templateId=MAKAAN_CITY_LISTING_BUY&page=1',
    ]
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 3,
            ...
            try:
                ...
                if item['Status'] == '' or item['Status'] is None:
                    ...
                    if aval is not None:
                        ...
                        item['Status'] = i.xpath(
                            './/*[contains(@class,"dcol furnishstatus")]/div[1]/text()').extract_first(default='None')
                    else:
                        item['Status'] = 'None'
                '''
                try:
                ...
                try:
                    ...
                    if pages is not None:
                        ...
                        if pageno < totalpages:
                            url = 'https://www.makaan.com/listings?sortBy=date-desc&listingType=rent&pageType=LISTINGS_PROPERTY_URLS&cityName=Kolkata&cityId=16&templateId=MAKAAN_CITY_LISTING_BUY&page=' + str(pageno+1)
