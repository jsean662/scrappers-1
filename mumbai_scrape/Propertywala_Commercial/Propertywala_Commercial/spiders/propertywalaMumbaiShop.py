# -*- coding: utf-8 -*-
import scrapy
from ..items import PropertywalaCommercialItem
...
class PropertywalamumbaiSpider(scrapy.Spider):
    name = "propertywalaMumbaiShop"
    allowed_domains = ["propertywala.com"]
    start_urls = [
        'https://www.propertywala.com/properties/type-commercial_shop/for-sale/location-mumbai_maharashtra?orderby=date',
        'https://www.propertywala.com/properties/type-commercial_shop/for-rent/location-mumbai_maharashtra?orderby=date',
        'https://www.propertywala.com/properties/type-commercial_showroom/for-sale/location-mumbai_maharashtra?orderby=date',
        'https://www.propertywala.com/properties/type-commercial_showroom/for-rent/location-mumbai_maharashtra?orderby=date',
    ]
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 5.0,
    ...
    item = PropertywalaCommercialItem()
    ...
    def parse1(self, response):
        ...
        if 'Shop' or 'Showroom' in conf:
            self.item['property_type'] = 'Shop'
        ...
        try:
            pincode = re.findall('[0-9]+', address[len(address)-1])
            self.item['areacode'] = str(pincode[0])
        except Exception as e:
            print(e)

        if self.item['areacode'] == '' or self.item['areacode'] == ' ' or self.item['areacode'] is None:
            self.item['areacode'] = '0'
        ...
        if 'ale' in self.item['txn_type']:
            ...
            if (('Within' in poss) and ('Month' in poss)):
                ...
                self.item['Status'] = 'Under Construction'
        elif 'ent' in self.item['txn_type']:
            self.item['Status'] = response.xpath('//*[@id="PropertyAttributes"]/li[contains(text(),"Furnish")]/span/text()').extract_first(default='None')
        ...
        if ((not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0') and (not self.item['lat'] == '0')) or ((not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0') and (not self.item['lat'] == '0')) or ((not self.item['price_per_sqft'] == '0') and (not self.item['Bua_sqft'] == '0') and (not self.item['lat'] == '0')):
            self.item['quality4'] = 1
        elif ((not self.item['price_per_sqft'] == '0') and (not self.item['lat'] == '0')) or ((not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0') and (not self.item['lat'] == '0')) or ((not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0') and (not self.item['lat'] == '0')) or ((not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0')) or ((not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0')):
