...
class MagicrentmumbaiSpider(scrapy.Spider):
    name = "magicRentMumbaiShop"
    allowed_domains = ["magicbricks.com"]
    start_urls = [
        'http://www.magicbricks.com/property-for-rent/commercial-real-estate?proptype=Commercial-Shop,Commercial-Showroom&cityName=Mumbai&sortBy=mostRecent/Page-1',
    ]
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 5.0,
        ...
        try:
            ...
            for i in data:
                try:
                    ...
                    try:
                        status = i.xpath('.//div[contains(@class,"showOneLilner")]/text()').extract_first(default='None')[1].replace('\n', ' ')
                    except:
            ...
            #     if pageNo < pagecount:
            #         url = 'http://www.magicbricks.com/property-for-rent/commercial-real-estate?proptype=Commercial-Shop,Commercial-Showroom&cityName=Mumbai&sortBy=mostRecent/Page-' + str(pageNo + 1)
            #         # print(url)
