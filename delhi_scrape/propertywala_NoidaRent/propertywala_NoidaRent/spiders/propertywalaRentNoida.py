# -*- coding: utf-8 -*-
import scrapy
from ..items import PropertywalaNoidarentItem
...
class PropertywalarentnoidaSpider(scrapy.Spider):
    name = "propertywalaRentNoida"
    allowed_domains = ["propertywala.com"]
    start_urls = (
        'https://www.propertywala.com/properties/type-residential/for-rent/location-noida_uttar_pradesh?pah=%s' % page for page in range(1, 35)
    )
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 1}
    item = PropertywalaNoidarentItem()
        ...
        if 'plot' in checkprop.lower() or 'land' in checkprop.lower():
            ...
            if 'House' in conf:
                ...
                if (self.item['city'] in self.item['Building_name']) or (
                            ...
                            self.item['Building_name'] in self.item['locality']) or (
                    ' for ' in self.item['Building_name']):
