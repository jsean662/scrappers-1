...
import scrapy
from acersRentDelhi.items import AcersrentdelhiItem
...
class AcresrentdelhiSpider(scrapy.Spider):
    name = "acresRentDelhi"
    allowed_domains = ["http://www.99acres.com/"]
    start_urls = ['http://www.99acres.com/rent-property-in-delhi-ncr-ffid-page-%s?orig_property_type=1,4,2,90&search_type=QS&search_location=CP12&lstacn=SEARCH&pageid=QS&src=PAGING&property_type=1,4,2,90/'% page for page in xrange(1,1958)]
    custom_settings = {
            'DEPTH_LIMIT': 3000,
            'DOWNLOAD_DELAY': 12
        }
