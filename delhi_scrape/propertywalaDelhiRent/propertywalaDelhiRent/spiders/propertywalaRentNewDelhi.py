# -*- coding: utf-8 -*-
import scrapy
from ..items import PropertywaladelhirentItem
...
class PropertywalarentnewdelhiSpider(scrapy.Spider):
    name = "propertywalaRentNewDelhi"
    allowed_domains = ["propertywala.com"]
    start_urls = (
        'https://www.propertywala.com/properties/type-residential/for-rent/location-new_delhi?page=%s' % page for page in range(1, 41)
    )
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 1
    }
    item = PropertywaladelhirentItem()
    ...
    def parse(self, response):
        ...
        for i in data:
            ...
            url = ('https://www.propertywala.com/' + i.xpath('text()').extract_first().strip().encode('ascii','ignore').decode('ascii').replace('ID: ', '').replace('  Posted:', ''))  # + nexch_data
    ...
    def parse1(self, response):
        ...
        if 'plot' in checkprop.lower() or 'land' in checkprop.lower():
            ...
            try:
                ...
                address = response.xpath('//div[@id="PropertyDetails"]/section/header/h4/text()').extract_first().strip()
                self.item['address'] = address
            except:
                self.item['address'] = 'Delhi'
            try:
                buildname = response.xpath('//*[@id="PropertySummary"]/header/h4/text()').extract_first().split(',')[0].strip()  # re.findall('for rent in (.*)?,', conf)
            except:
