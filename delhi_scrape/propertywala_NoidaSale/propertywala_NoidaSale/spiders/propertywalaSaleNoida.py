# -*- coding: utf-8 -*-
import scrapy
from ..items import PropertywalaNoidasaleItem
...
class PropertywalasalenoidaSpider(scrapy.Spider):
    name = "propertywalaSaleNoida"
    allowed_domains = ["propertywala.com"]
    start_urls = (
        'https://www.propertywala.com/properties/type-residential/for-sale/location-noida_uttar_pradesh?page=%s' % page for page in range(1, 41)
    )
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 1}
    item = PropertywalaNoidasaleItem()
    ...
    def parse1(self, response):
        ...
        if 'plot' in checkprop.lower() or 'land' in checkprop.lower():
            ...
            dat = response.xpath('//li[@class="noPrint"]/time/@datetime').extract_first(default='0').replace('Z', '')
            ...
            try:
                loc = response.xpath(
                    '//div[@id="PropertyDetails"]/section/header/h4/text()').extract_first().strip().split(',')
                ...
                self.item['address'] = 'Noida'
                self.item['locality'] = 'None'

            try:
                buildname = response.xpath('//*[@id="PropertySummary"]/header/h4').extract_first().split(',')[
                    0]  # re.findall('for rent in (.*)?,', conf)
            except:
            ...
            if ' rent ' in conf.lower():
                self.item['txn_type'] = 'Rent'
            if ' sale ' in conf.lower():
                try:
            ...
            try:
                ...
                pass
            try:
                price_square = response.xpath(
                    "//ul[@id='PropertyAttributes']/li[contains(text(),'Rate')]/span/text()").extract_first(
                    default='0').replace(',', '')
            ...
            try:
                self.item['Bua_sqft'] = \
                response.xpath('//span[@class="areaUnit downArrow"]/text()').extract_first(default='0').split(' ')[0]
            except:
            ...
            if (((not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0') and (
            not self.item['Building_name'] == 'None') and (not self.item['lat'] == '0')) or (
                        (not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0') and (
                not self.item['Building_name'] == 'None') and (not self.item['lat'] == '0')) or (
                        (not self.item['price_per_sqft'] == '0') and (not self.item['Bua_sqft'] == '0') and (
                not self.item['Building_name'] == 'None') and (not self.item['lat'] == '0'))):
                self.item['quality4'] = 1
            elif (((not self.item['price_per_sqft'] == '0') and (not self.item['Building_name'] == 'None') and (
            not self.item['lat'] == '0')) or (
                    (not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0') and (
            not self.item['lat'] == '0')) or (
                    (not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0') and (
            not self.item['lat'] == '0')) or (
                    (not self.item['Selling_price'] == '0') and (not self.item['Bua_sqft'] == '0') and (
            not self.item['Building_name'] == 'None')) or (
                    (not self.item['Monthly_Rent'] == '0') and (not self.item['Bua_sqft'] == '0') and (
            not self.item['Building_name'] == 'None'))):
                self.item['quality4'] = 0.5
            else:
                self.item['quality4'] = 0
            if ((not self.item['Building_name'] == 'None') and (not self.item['listing_date'] == '0') and (
            not self.item['txn_type'] == 'None') and (not self.item['property_type'] == 'None') and (
                (not self.item['Selling_price'] == '0') or (not self.item['Monthly_Rent'] == '0'))):
