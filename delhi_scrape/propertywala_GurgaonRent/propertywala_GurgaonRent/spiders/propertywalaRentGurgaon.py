# -*- coding: utf-8 -*-
import scrapy
from ..items import PropertywalaGurgaonrentItem
...
class PropertywalarentgurgaonSpider(scrapy.Spider):
    name = "propertywalaRentGurgaon"
    allowed_domains = ["propertywala.com"]
    start_urls = [
        'https://www.propertywala.com/properties/type-residential/for-rent/location-gurgaon_haryana?page=%s' % page for page in range(1, 41)
    ]
    custom_settings = {
        'DEPTH_LIMIT': 10000,
        'DOWNLOAD_DELAY': 1}
    item = PropertywalaGurgaonrentItem()
    ...
    def parse(self, response):
        ...
        for i in data:
            ...
            url = 'https://www.propertywala.com/' + i.xpath('text()').extract_first().strip().encode('ascii',
                                                                                                     'ignore').decode(
                'ascii').replace('ID: ', '').replace('  Posted:', '')
    ...
    def parse1(self, response):
        ...
        if 'plot' in checkprop.lower() or 'land' in checkprop.lower():
            ...
            try:
                ...
                self.item['address'] = 'Gurgaon'
                self.item['locality'] = 'None'

            try:
                buildname = loc[0]  # response.xpath('//*[@id="PropertySummary"]/header/h4/text()').extract_first().split(',')[0].strip()
            ...
            if 'House' in conf:
                ...
                if self.item['locality'] == self.item['Building_name']:
                    self.item['Building_name'] = 'None'
            ...
            if ((not self.item['Building_name'] == 'None') and (not self.item['listing_date'] == '0') and (not self.item['txn_type'] == 'None') and (not self.item['property_type'] == 'None') and (
