...
class PropWala(CrawlSpider):
	...
	def parse1(self,response):
		...
		if (('Apartment / Flat' in conf) or ('Independent' in conf)):
			self.item['config_type'] = conf.strip().split(' ')[0]+' BHK'
		if 'Plot / Land' in conf:
			self.item['config_type'] = 'None'

		build = response.xpath('//div[@id="PropertyDetails"]/section/header/h3/text()').extract_first()
		if ((',' in build) and ('in ' in build)):
			self.item['Building_name'] = build.split(',')[0].split('in ')[-1]
		else:
